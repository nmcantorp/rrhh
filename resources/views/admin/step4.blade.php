...
{{ Form::open(['route' => 'admin.users.step4store', 'method'=>'POST', 'files' => true]) }}
...
<div id="details">	
	...
	<div class="row">
	    <div class="five columns">
			...
			{{ Form::select('nombre_inst[]', $institucion, null,['class'=>'smoothborder form-control', 'required', 'placeholder'=>'--Selecciona Intitución--'] ) }}
	    </div>
	    <div class="five columns">
	    	{{ Form::label('tipo_for', 'Tipo de Formación')}} 	                                    	                   
			{{ Form::select('tipo_for[]', $tipo_formacion, null,['class'=>'smoothborder form-control', 'required', 'placeholder'=>'--Selecciona tipo formación--'] ) }}
	    </div>
	    <div class="two columns">
	    	{{ Form::label('anio', 'Año Egresado')}} 	                                    	                   
			{{ Form::text('anio[]', null, ['class'=>'smoothborder', 'required', 'placeholder'=>'xxxx'] ) }}
	    </div>	     
	</div>
	<div class="row">
	    <div class="six columns">
	    	{{ Form::label('titulo', 'Titulo Profesional')}} 	                                    	                   
			{{ Form::text('titulo[]', null, ['class'=>'smoothborder', 'required', 'placeholder'=>'Titulo Profesional'] ) }}
	    </div>
	    <div class="four columns">
	    	{{ Form::label('titulo_obt', 'Titulo Obtenido')}} 	                                    	                   
			{{ Form::select('titulo_obt[]', $area_conocimiento, null,['class'=>'smoothborder form-control', 'required', 'placeholder'=>'--Selecciona Titulo--'] ) }}
	    </div>
	</div>
